...
noise = sig*randl(II); % generating noise
...
delta = 0.8 * (1 - missing_rate) * (sig/dom(2)) * prod(II); % controling noise inequality
...
[X, histo, GAM1, GAM2, ppk, ddk, Cp, Cd] = LRTV_l1_pds_ada_skipobj(Tms/dom(2),Q,al,tv,be,lr,delta,dom/dom(2),Gam,rho,maxiter,tol,verb);

% It does not skip to evaluate objective function for each step that requires several times of SVD
%[X, histo, GAM1, GAM2, ppk, ddk, Cp, Cd, histo_obj] = LRTV_l1_pds_ada(Tms/dom(2),Q,al,tv,be,lr,delta,dom/dom(2),Gam,rho,maxiter,tol,verb);
