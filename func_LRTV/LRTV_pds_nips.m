% LRTV-PDS integrated with step size adaptation that is proposed by Goldstein
% Goldstein et al, "Adaptive Primal-Dual Splitting Methods for Statistical
% Learning and Image Processing", in NeurIPS2015.
function [X, histo, GAM1, GAM2, PPk, DDk, Cbk, histo_obj] = LRTV_pds_nips(T,Q,al,w,be,lam,delta,dom,Gam,maxiter,tol,verb)
  ...
  for iter = 1:maxiter
    ...
    if mod(iter,verb) == 0
      fprintf('%d :: condintion(%e) :: pdr(%e) :: gam(%e,%e), \n',iter,condition,pdr(iter),gam1,gam2);
    end
    if condition < tol
      break;
    end
    ...
    %% Goldstein's adaptation
      ...
      dy2 = norm(Vdif(:))^2;
      AtY = Pk - Xdif/gam1;
      Cbk(iter) = c_nips/2/gam1*dx2 + c_nips/2/gam2*dy2 - 2*inp(AtY,Xdif);
    %%
    ...
    if 2*PPk(iter) < DDk(iter)
      gam1 = gam1*(1-alpha_nips);
      gam2 = gam2/(1-alpha_nips);
      alpha_nips = alpha_nips*eta_nips;
    end
    if PPk(iter) > 2*DDk(iter)
      gam1 = gam1/(1-alpha_nips);
      gam2 = gam2*(1-alpha_nips);
      alpha_nips = alpha_nips*eta_nips;
    end
